...
typedef struct _GskVkOldParseState GskVkOldParseState;

struct _GskVkOldRenderPass
{
  int empty;
};

struct _GskVkOldParseState
{
...
GskVkOldRenderPass *
gsk_vk_old_render_pass_new (void)
{
  GskVkOldRenderPass *self;

  self = g_new0 (GskVkOldRenderPass, 1);

  return self;
}

void
gsk_vk_old_render_pass_free (GskVkOldRenderPass *self)
{
  g_free (self);
}

static void
gsk_vk_old_render_pass_append_scissor (GskVkOldRender           *render,
                                       GskRenderNode             *node,
                                       const GskVkOldParseState *state)
{
  gsk_vk_old_scissor_op (render, &state->scissor);
}

static void
gsk_vk_old_render_pass_append_push_constants (GskVkOldRender           *render,
                                              GskRenderNode             *node,
                                              const GskVkOldParseState *state)
{
  ...
  gsk_vk_old_push_constants_op (render, &state->scale, &mvp, &state->clip.rect);
}

#define FALLBACK(...) G_STMT_START { \
  GSK_RENDERER_DEBUG (gsk_vk_old_render_get_renderer (render), FALLBACK, __VA_ARGS__); \
  return FALSE; \
}G_STMT_END

static gboolean
gsk_vk_old_parse_rect_is_integer (const GskVkOldParseState *state,
...
gsk_vk_old_render_pass_upload_texture (GskVkOldRender *render,
                                       GdkTexture      *texture)
{
  GskVkOldImage *image, *better_image;
  int width, height;
  GskVkOldImagePostprocess postproc;
  graphene_matrix_t projection;
  graphene_vec2_t scale;

  image = gsk_vk_old_upload_texture_op (render, texture);
  postproc = gsk_vk_old_image_get_postprocess (image);
  if (postproc == 0)
    return image;

  width = gdk_texture_get_width (texture);
  height = gdk_texture_get_height (texture);
  better_image = gsk_vk_old_image_new_for_offscreen (gsk_vk_old_render_get_context (render),
  ...
  gsk_vk_old_scissor_op (render, &(cairo_rectangle_int_t) { 0, 0, width, height });
  ...
  gsk_vk_old_push_constants_op (render, &scale, &projection, &GSK_ROUNDED_RECT_INIT(0, 0, width, height));
  gsk_vk_old_convert_op (render,
...
static GskVkOldImage *
gsk_vk_old_render_pass_get_node_as_image (GskVkOldRenderPass       *self,
                                          GskVkOldRender           *render,
                                          const GskVkOldParseState *state,
                                          GskRenderNode             *node,
                                          graphene_rect_t           *tex_bounds)
{
  GskVkOldImage *result;
    ...
    case GSK_TEXTURE_NODE:
      {
        ...
        GskVkOldRenderer *renderer = GSK_VK_OLD_RENDERER (gsk_vk_old_render_get_renderer (render));
        result = gsk_vk_old_renderer_get_texture_image (renderer, texture);
        if (result == NULL)
          {
            result = gsk_vk_old_render_pass_upload_texture (render, texture);
            gsk_vk_old_renderer_add_texture_image (renderer, texture, result);
          }
    ...
    case GSK_CAIRO_NODE:
      {
        ...
        result = gsk_vk_old_upload_cairo_op (render,
                                             node,
                                             &state->scale,
                                             &clipped);
    ...
    default:
      {
        ...
        *tex_bounds = clipped;
        ...
        result = gsk_vk_old_render_pass_op_offscreen (render,
                                                      &state->scale,
                                                      &clipped,
                                                      node);
...
static void
gsk_vk_old_render_pass_add_node (GskVkOldRenderPass       *self,
                                 GskVkOldRender           *render,
                                 const GskVkOldParseState *state,
                                 GskRenderNode             *node);

static inline gboolean
gsk_vk_old_render_pass_add_fallback_node (GskVkOldRenderPass       *self,
                                          GskVkOldRender           *render,
                                          const GskVkOldParseState *state,
                                          GskRenderNode             *node)
{
  ...
  image = gsk_vk_old_upload_cairo_op (render,
                                      node,
                                      &state->scale,
                                      &clipped);

  gsk_vk_old_texture_op (render,
                         gsk_vk_old_clip_get_shader_clip (&state->clip, &state->offset, &node->bounds),
                         image,
...
static inline gboolean
gsk_vk_old_render_pass_implode (GskVkOldRenderPass       *self,
                                GskVkOldRender           *render,
                                const GskVkOldParseState *state,
                                GskRenderNode             *node)
{
...
static inline gboolean
gsk_vk_old_render_pass_add_container_node (GskVkOldRenderPass       *self,
                                           GskVkOldRender           *render,
                                           const GskVkOldParseState *state,
                                           GskRenderNode             *node)
{
  for (guint i = 0; i < gsk_container_node_get_n_children (node); i++)
    gsk_vk_old_render_pass_add_node (self, render, state, gsk_container_node_get_child (node, i));

  return TRUE;
}

static inline gboolean
gsk_vk_old_render_pass_add_cairo_node (GskVkOldRenderPass       *self,
                                       GskVkOldRender           *render,
                                       const GskVkOldParseState *state,
                                       GskRenderNode             *node)
{
  ...
  if (gsk_cairo_node_get_surface (node) != NULL)
    return gsk_vk_old_render_pass_add_fallback_node (self, render, state, node);

  return TRUE;
}

static inline gboolean
gsk_vk_old_render_pass_add_color_node (GskVkOldRenderPass       *self,
                                       GskVkOldRender           *render,
                                       const GskVkOldParseState *state,
                                       GskRenderNode             *node)
{
  ...
  if (gdk_rgba_is_opaque (color) &&
      ...
      gsk_vk_old_parse_rect_is_integer (state, &clipped, &int_clipped))
    {
      ...
      if (state->clip.type == GSK_VK_OLD_CLIP_ROUNDED)
        {
          graphene_rect_t cover;
          GskVkOldShaderClip shader_clip;
          float scale_x = graphene_vec2_get_x (&state->scale);
          float scale_y = graphene_vec2_get_y (&state->scale);
          ...
          shader_clip = gsk_vk_old_clip_get_shader_clip (&state->clip, graphene_point_zero(), &clipped);
          if (shader_clip != GSK_VK_OLD_SHADER_CLIP_NONE)
            {
...
static inline gboolean
gsk_vk_old_render_pass_add_linear_gradient_node (GskVkOldRenderPass       *self,
                                                 GskVkOldRender           *render,
                                                 const GskVkOldParseState *state,
                                                 GskRenderNode             *node)
{
  gsk_vk_old_linear_gradient_op (render,
                                 gsk_vk_old_clip_get_shader_clip (&state->clip, &state->offset, &node->bounds),
                                 &node->bounds,
                                 &state->offset,
...
static inline gboolean
gsk_vk_old_render_pass_add_border_node (GskVkOldRenderPass       *self,
                                        GskVkOldRender           *render,
                                        const GskVkOldParseState *state,
                                        GskRenderNode             *node)
{
  gsk_vk_old_border_op (render,
                        gsk_vk_old_clip_get_shader_clip (&state->clip, &state->offset, &node->bounds),
...
static inline gboolean
gsk_vk_old_render_pass_add_texture_node (GskVkOldRenderPass       *self,
                                         GskVkOldRender           *render,
                                         const GskVkOldParseState *state,
                                         GskRenderNode             *node)
{
  ...
  renderer = GSK_VK_OLD_RENDERER (gsk_vk_old_render_get_renderer (render));
  texture = gsk_texture_node_get_texture (node);
  image = gsk_vk_old_renderer_get_texture_image (renderer, texture);
  if (image == NULL)
    {
      image = gsk_vk_old_render_pass_upload_texture (render, texture);
      gsk_vk_old_renderer_add_texture_image (renderer, texture, image);
    }
...
static inline gboolean
gsk_vk_old_render_pass_add_texture_scale_node (GskVkOldRenderPass       *self,
                                               GskVkOldRender           *render,
                                               const GskVkOldParseState *state,
                                               GskRenderNode             *node)
{
  ...
  switch (gsk_texture_scale_node_get_filter (node))
    ...
    case GSK_SCALING_FILTER_TRILINEAR:
      sampler = GSK_VK_OLD_SAMPLER_DEFAULT;
      break;
    case GSK_SCALING_FILTER_NEAREST:
      sampler = GSK_VK_OLD_SAMPLER_NEAREST;
      break;
    }
  image = gsk_vk_old_renderer_get_texture_image (renderer, texture);
  if (image == NULL)
...
static inline gboolean
gsk_vk_old_render_pass_add_inset_shadow_node (GskVkOldRenderPass       *self,
                                              GskVkOldRender           *render,
                                              const GskVkOldParseState *state,
                                              GskRenderNode             *node)
{
  ...
  gsk_vk_old_inset_shadow_op (render,
                              gsk_vk_old_clip_get_shader_clip (&state->clip, &state->offset, &node->bounds),
...
static inline gboolean
gsk_vk_old_render_pass_add_outset_shadow_node (GskVkOldRenderPass       *self,
                                               GskVkOldRender           *render,
                                               const GskVkOldParseState *state,
                                               GskRenderNode             *node)
{
  ...
  gsk_vk_old_outset_shadow_op (render,
                               gsk_vk_old_clip_get_shader_clip (&state->clip, &state->offset, &node->bounds),
...
static inline gboolean
gsk_vk_old_render_pass_add_transform_node (GskVkOldRenderPass       *self,
                                           GskVkOldRender           *render,
                                           const GskVkOldParseState *state,
                                           GskRenderNode             *node)
{
    ...
    case GSK_TRANSFORM_CATEGORY_2D_TRANSLATE:
      {
        ...
        gsk_vk_old_render_pass_add_node (self, render, &new_state, child);
      }
      return TRUE;
    ...
    case GSK_TRANSFORM_CATEGORY_2D_AFFINE:
      {
        ...
        gsk_vk_old_clip_scale (&new_state.clip, &state->clip, scale_x, scale_y);
    ...
    case GSK_TRANSFORM_CATEGORY_2D:
      {
        ...
        if (gsk_vk_old_clip_contains_rect (&state->clip, &state->offset, &node->bounds))
          {
            gsk_vk_old_clip_init_empty (&new_state.clip, &child->bounds);
          }
        else if (!gsk_vk_old_clip_transform (&new_state.clip, &state->clip, clip_transform, &child->bounds))
          {
  ...
  gsk_vk_old_render_pass_append_push_constants (render, node, &new_state);

  gsk_vk_old_render_pass_add_node (self, render, &new_state, child);

  gsk_vk_old_render_pass_append_push_constants (render, node, state);
...
static gboolean
gsk_vk_old_render_pass_add_opacity_node (GskVkOldRenderPass       *self,
                                         GskVkOldRender           *render,
                                         const GskVkOldParseState *state,
                                         GskRenderNode             *node)
{
  ...
  image = gsk_vk_old_render_pass_get_node_as_image (self,
                                                    render,
                                                    state,
  ...
  gsk_vk_old_color_matrix_op_opacity (render,
                                      gsk_vk_old_clip_get_shader_clip (&state->clip, &state->offset, &node->bounds),
                                      image,
                                      &node->bounds,
                                      &state->offset,
...
static inline gboolean
gsk_vk_old_render_pass_add_color_matrix_node (GskVkOldRenderPass       *self,
                                              GskVkOldRender           *render,
                                              const GskVkOldParseState *state,
                                              GskRenderNode             *node)
{
...
static inline gboolean
gsk_vk_old_render_pass_add_clip_node (GskVkOldRenderPass       *self,
                                      GskVkOldRender           *render,
                                      const GskVkOldParseState *state,
                                      GskRenderNode             *node)
{
  ...
  if (gsk_vk_old_parse_rect_is_integer (state, &clip, &new_state.scissor))
    {
      if (!gdk_rectangle_intersect (&new_state.scissor, &state->scissor, &new_state.scissor))
        return TRUE;

      if (gsk_vk_old_clip_intersect_rect (&new_state.clip, &state->clip, &clip))
        {
          if (new_state.clip.type == GSK_VK_OLD_CLIP_RECT)
            new_state.clip.type = GSK_VK_OLD_CLIP_NONE;

          do_push_constants = TRUE;
        }
      else
        {
          gsk_vk_old_clip_init_copy (&new_state.clip, &state->clip);
  ...
  if (new_state.clip.type == GSK_VK_OLD_CLIP_ALL_CLIPPED)
    return TRUE;
  ...
  if (do_scissor)
    gsk_vk_old_render_pass_append_scissor (render, node, &new_state);
  if (do_push_constants)
    gsk_vk_old_render_pass_append_push_constants (render, node, &new_state);

  gsk_vk_old_render_pass_add_node (self, render, &new_state, gsk_clip_node_get_child (node));

  if (do_push_constants)
    gsk_vk_old_render_pass_append_push_constants (render, node, state);
  if (do_scissor)
    gsk_vk_old_render_pass_append_scissor (render, node, state);

  return TRUE;
}

static inline gboolean
gsk_vk_old_render_pass_add_rounded_clip_node (GskVkOldRenderPass       *self,
                                              GskVkOldRender           *render,
                                              const GskVkOldParseState *state,
                                              GskRenderNode             *node)
{
  ...
  if (!gsk_vk_old_clip_intersect_rounded_rect (&new_state.clip, &state->clip, &clip))
  ...
  gsk_vk_old_render_pass_add_node (self, render, &new_state, gsk_rounded_clip_node_get_child (node));
...
static inline gboolean
gsk_vk_old_render_pass_add_repeat_node (GskVkOldRenderPass       *self,
                                        GskVkOldRender           *render,
                                        const GskVkOldParseState *state,
                                        GskRenderNode             *node)
{
  ...
  image = gsk_vk_old_render_pass_op_offscreen (render,
                                               &state->scale,
                                               child_bounds,
...
static inline gboolean
gsk_vk_old_render_pass_add_blend_node (GskVkOldRenderPass       *self,
                                       GskVkOldRender           *render,
                                       const GskVkOldParseState *state,
                                       GskRenderNode             *node)
{
  GskRenderNode *top_child, *bottom_child;
  GskVkOldImage *top_image, *bottom_image;
  graphene_rect_t top_tex_rect, bottom_tex_rect;

  top_child = gsk_blend_node_get_top_child (node);
  top_image = gsk_vk_old_render_pass_get_node_as_image (self,
                                                        render,
                                                        state,
                                                        top_child,
                                                        &top_tex_rect);
  bottom_child = gsk_blend_node_get_bottom_child (node);
  bottom_image = gsk_vk_old_render_pass_get_node_as_image (self,
                                                           render,
                                                           state,
                                                           bottom_child,
                                                           &bottom_tex_rect);
  ...
  gsk_vk_old_blend_mode_op (render,
                            gsk_vk_old_clip_get_shader_clip (&state->clip, &state->offset, &node->bounds),
                            &node->bounds,
                            &state->offset,
...
static inline gboolean
gsk_vk_old_render_pass_add_cross_fade_node (GskVkOldRenderPass       *self,
                                            GskVkOldRender           *render,
                                            const GskVkOldParseState *state,
                                            GskRenderNode             *node)
{
  GskRenderNode *start_child, *end_child;
  GskVkOldImage *start_image, *end_image;
  ...
  start_image = gsk_vk_old_render_pass_get_node_as_image (self,
                                                          render,
                                                          state,
                                                          start_child,
                                                          &start_tex_rect);
  end_child = gsk_cross_fade_node_get_end_child (node);
  end_image = gsk_vk_old_render_pass_get_node_as_image (self,
                                                        render,
                                                        state,
                                                        end_child,
                                                        &end_tex_rect);
  if (start_image == NULL)
    {
      ...
      gsk_vk_old_color_matrix_op_opacity (render,
                                          gsk_vk_old_clip_get_shader_clip (&state->clip, &state->offset, &end_child->bounds),
                                          end_image,
                                          &node->bounds,
                                          &state->offset,
                                          &end_tex_rect,
    ...
    {
      gsk_vk_old_color_matrix_op_opacity (render,
                                          gsk_vk_old_clip_get_shader_clip (&state->clip, &state->offset, &start_child->bounds),
                                          start_image,
                                          &node->bounds,
                                          &state->offset,
                                          &start_tex_rect,
  ...
  gsk_vk_old_cross_fade_op (render,
                            gsk_vk_old_clip_get_shader_clip (&state->clip, &state->offset, &node->bounds),
                            &node->bounds,
                            &state->offset,
                            progress,
...
static inline gboolean
gsk_vk_old_render_pass_add_text_node (GskVkOldRenderPass       *self,
                                      GskVkOldRender           *render,
                                      const GskVkOldParseState *state,
                                      GskRenderNode             *node)
{
  ...
  cache = gsk_vk_old_renderer_get_glyph_cache (GSK_VK_OLD_RENDERER (gsk_vk_old_render_get_renderer (render)));
  ...
  for (i = 0; i < num_glyphs; i++)
    {
      GskVkOldCachedGlyph *glyph;
      const PangoGlyphInfo *gi = &glyphs[i];
      graphene_rect_t glyph_bounds, glyph_tex_rect;

      glyph = gsk_vk_old_glyph_cache_lookup (cache,
                                             render,
                                             (PangoFont *)font,
                                             gi->glyph,
      ...
      if (gsk_text_node_has_color_glyphs (node))
        gsk_vk_old_texture_op (render,
                               gsk_vk_old_clip_get_shader_clip (&state->clip, &state->offset, &glyph_bounds),
                               glyph->atlas_image,
                               GSK_VK_OLD_SAMPLER_DEFAULT,
                               &glyph_bounds,
                               &state->offset,
...
static inline gboolean
gsk_vk_old_render_pass_add_blur_node (GskVkOldRenderPass       *self,
                                      GskVkOldRender           *render,
                                      const GskVkOldParseState *state,
                                      GskRenderNode             *node)
{
  ...
  if (radius == 0)
    {
      gsk_vk_old_render_pass_add_node (self, render, state, gsk_blur_node_get_child (node));
      return TRUE;
    }
...
static inline gboolean
gsk_vk_old_render_pass_add_mask_node (GskVkOldRenderPass       *self,
                                      GskVkOldRender           *render,
                                      const GskVkOldParseState *state,
                                      GskRenderNode             *node)
{
  GskVkOldImage *source_image, *mask_image;
  ...
  mask_image = gsk_vk_old_render_pass_get_node_as_image (self,
                                                         render,
                                                         state,
                                                         mask,
                                                         &mask_tex_rect);
  if (mask_image == NULL)
    {
      if (mode == GSK_MASK_MODE_INVERTED_ALPHA)
        gsk_vk_old_render_pass_add_node (self, render, state, source);
  ...
  if (mode == GSK_MASK_MODE_ALPHA &&
      gsk_render_node_get_node_type (source) == GSK_COLOR_NODE)
    {
      ...
      if (graphene_rect_intersection (&source->bounds, &mask->bounds, &bounds))
        gsk_vk_old_glyph_op (render,
                             gsk_vk_old_clip_get_shader_clip (&state->clip, &state->offset, &bounds),
                             mask_image,
                             &bounds,
                             &state->offset,
                             &mask_tex_rect,
  ...
  source_image = gsk_vk_old_render_pass_get_node_as_image (self,
                                                           render,
                                                           state,
                                                           source,
                                                           &source_tex_rect);
...
static inline gboolean
gsk_vk_old_render_pass_add_debug_node (GskVkOldRenderPass       *self,
                                       GskVkOldRender           *render,
                                       const GskVkOldParseState *state,
                                       GskRenderNode             *node)
{
  gsk_vk_old_render_pass_add_node (self, render, state, gsk_debug_node_get_child (node));
  return TRUE;
}

#undef FALLBACK

typedef gboolean (*GskVkOldRenderPassNodeFunc) (GskVkOldRenderPass       *self,
                                                 GskVkOldRender           *render,
                                                 const GskVkOldParseState *state,
                                                 GskRenderNode             *node);

/* TODO: implement remaining nodes */
static const GskVkOldRenderPassNodeFunc nodes_vtable[] = {
  [GSK_NOT_A_RENDER_NODE] = gsk_vk_old_render_pass_implode,
  [GSK_CONTAINER_NODE] = gsk_vk_old_render_pass_add_container_node,
  [GSK_CAIRO_NODE] = gsk_vk_old_render_pass_add_cairo_node,
  [GSK_COLOR_NODE] = gsk_vk_old_render_pass_add_color_node,
  [GSK_LINEAR_GRADIENT_NODE] = gsk_vk_old_render_pass_add_linear_gradient_node,
  [GSK_REPEATING_LINEAR_GRADIENT_NODE] = gsk_vk_old_render_pass_add_linear_gradient_node,
  [GSK_RADIAL_GRADIENT_NODE] = NULL,
  [GSK_REPEATING_RADIAL_GRADIENT_NODE] = NULL,
  [GSK_CONIC_GRADIENT_NODE] = NULL,
  [GSK_BORDER_NODE] = gsk_vk_old_render_pass_add_border_node,
  [GSK_TEXTURE_NODE] = gsk_vk_old_render_pass_add_texture_node,
  [GSK_INSET_SHADOW_NODE] = gsk_vk_old_render_pass_add_inset_shadow_node,
  [GSK_OUTSET_SHADOW_NODE] = gsk_vk_old_render_pass_add_outset_shadow_node,
  [GSK_TRANSFORM_NODE] = gsk_vk_old_render_pass_add_transform_node,
  [GSK_OPACITY_NODE] = gsk_vk_old_render_pass_add_opacity_node,
  [GSK_COLOR_MATRIX_NODE] = gsk_vk_old_render_pass_add_color_matrix_node,
  [GSK_REPEAT_NODE] = gsk_vk_old_render_pass_add_repeat_node,
  [GSK_CLIP_NODE] = gsk_vk_old_render_pass_add_clip_node,
  [GSK_ROUNDED_CLIP_NODE] = gsk_vk_old_render_pass_add_rounded_clip_node,
  [GSK_SHADOW_NODE] = NULL,
  [GSK_BLEND_NODE] = gsk_vk_old_render_pass_add_blend_node,
  [GSK_CROSS_FADE_NODE] = gsk_vk_old_render_pass_add_cross_fade_node,
  [GSK_TEXT_NODE] = gsk_vk_old_render_pass_add_text_node,
  [GSK_BLUR_NODE] = gsk_vk_old_render_pass_add_blur_node,
  [GSK_DEBUG_NODE] = gsk_vk_old_render_pass_add_debug_node,
  [GSK_GL_SHADER_NODE] = NULL,
  [GSK_TEXTURE_SCALE_NODE] = gsk_vk_old_render_pass_add_texture_scale_node,
  [GSK_MASK_NODE] = gsk_vk_old_render_pass_add_mask_node,
  [GSK_FILL_NODE] = NULL,
...
gsk_vk_old_render_pass_add_node (GskVkOldRenderPass       *self,
                                 ...
                                 GskRenderNode             *node)
{
  GskVkOldRenderPassNodeFunc node_func;
  GskRenderNodeType node_type;
  gboolean fallback = FALSE;

  /* This catches the corner cases of empty nodes, so after this check
   * there's quaranteed to be at least 1 pixel that needs to be drawn */
  if (!gsk_vk_old_clip_may_intersect_rect (&state->clip, &state->offset, &node->bounds))
    return;
  ...
  if (node_func)
    ...
    {
      GSK_RENDERER_DEBUG (gsk_vk_old_render_get_renderer (render),
  ...
  if (fallback)
    gsk_vk_old_render_pass_add_fallback_node (self, render, state, node);
}

void
gsk_vk_old_render_pass_add (GskVkOldRenderPass   *self,
                            GskVkOldRender       *render,
                            int                    width,
                            int                    height,
...
{
  GskVkOldParseState state;

  state.scissor = *clip;
  gsk_vk_old_clip_init_empty (&state.clip, &GRAPHENE_RECT_INIT (0, 0, viewport->size.width, viewport->size.height));
  ...
  gsk_vk_old_render_pass_add_node (self, render, &state, node);
}
