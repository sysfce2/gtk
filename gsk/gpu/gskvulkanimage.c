                                 ...
                                 GdkTexture      *texture)
{
  ...
  int fd;
  const GdkVulkanDmabufFormat *format_entry;
  gsize width, height;
  ...
  format_entry = gdk_vulkan_dmabuf_find_format (dmabuf->fourcc);
  ...
  gdk_display_ref_vulkan (self->display);
  self->vk_tiling = VK_IMAGE_TILING_DRM_FORMAT_MODIFIER_EXT;
  self->vk_format = format_entry->vk_format;
  self->vk_pipeline_stage = VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT;
  self->vk_image_layout = VK_IMAGE_LAYOUT_UNDEFINED;
  self->vk_access = 0;
  ...
  gsk_gpu_image_setup (GSK_GPU_IMAGE (self),
                       ...
                       (format_entry->is_yuv ? (GSK_GPU_IMAGE_EXTERNAL | GSK_GPU_IMAGE_NO_BLIT) : 0),
  ...
  if (format_entry->is_yuv)
    vk_conversion = gsk_vulkan_device_get_vk_conversion (device, format_entry->vk_format, &self->vk_sampler);
  else
...
gsk_vulkan_image_to_dmabuf_texture (GskVulkanImage *self)
{
  ...
  if (!gdk_dmabuf_get_fourcc (gsk_gpu_image_get_format (image), &fourcc))
    return FALSE;
