                     ...
                     inout vec2 pos)
{
  ...
  pos = mod (pos - bounds.bounds.xy, rect_size (bounds)) + bounds.bounds.xy;
}

void
repeat_pop_pattern (inout uint reader,
                    inout vec2 pos)
{
  pos = stack_pop ().xy;
}
                        ...
                        vec2       pos)
{
  ...
  offset = fract (offset);
  float d_offset = 0.5 * min (fwidth (offset), fwidth (overflow));
  ...
  for(;;)
    {
      ...
      switch (type)
      {
        ...
        case GSK_GPU_PATTERN_REPEAT_PUSH:
          ...
          break;
        case GSK_GPU_PATTERN_REPEAT_POP:
          repeat_pop_pattern (reader, pos);
          break;
      }
    }
}
