...
gsk_ngl_renderer_get_backbuffer (GskGpuRenderer *renderer)
{
  ...
  if (self->backbuffer == NULL ||
      gsk_gpu_image_get_width (self->backbuffer) != gdk_surface_get_width (surface) ||
      gsk_gpu_image_get_height (self->backbuffer) != gdk_surface_get_height (surface))
    {
...
{
  gdk_gl_context_make_current (GDK_GL_CONTEXT (gsk_gpu_renderer_get_context (GSK_GPU_RENDERER (renderer))));

  return GSK_RENDERER_CLASS (gsk_ngl_renderer_parent_class)->render_texture (renderer, root, viewport);
}
